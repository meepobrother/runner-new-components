...
export class RunnerUtilService {
  ...
  addVersion() {
    ...
    version = version + 1;
    this.core.post('setting.save', { code: 'version', data: { version: version } })
      .subscribe(res => { });
  }
  ...
  getRole() {
    this.checkVersion().subscribe(res => {
      if (res) {
        const data = store.get('runner.my.getrole');
        if (data) {
          const info = data;
  ...
  checkMobile(cellPhone: string) {
    const regu = /^[S|U]((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(18[0,2,3,5-9]))\d{8}$/;
    const re = new RegExp(regu);
